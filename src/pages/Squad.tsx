...
import { User, RefreshCw, Search, ChevronRight } from "lucide-react";
              ...
              {filteredPlayers.map((player, index) => (
                ...
                >
                  <Link 
                    ...
                    className="group block relative bg-card-gradient border border-border rounded-xl overflow-hidden hover:border-primary/50 transition-all hover:shadow-lg"
                  >
                    {/* Captain/VC Badge */}
                    {player.is_captain && (
                    ...
                    <div className="p-5">
                      ...
                      {(player.battingStats || player.bowlingStats) && (
                        <div className="mt-4 pt-4 border-t border-border">
                          {player.battingStats && (player.battingStats.runs ?? 0) > 0 && (
                            <div className="flex justify-between text-sm">
                              <span className="text-muted-foreground">Runs</span>
                              <span className="font-semibold text-gold">{player.battingStats.runs}</span>
                            </div>
                          )}
                          {player.bowlingStats && (player.bowlingStats.wickets ?? 0) > 0 && (
                            <div className="flex justify-between text-sm">
                              <span className="text-muted-foreground">Wickets</span>
                              <span className="font-semibold text-primary">{player.bowlingStats.wickets}</span>
                            </div>
                          )}
                        </div>
                      )}
